...
class RNEventEmitter: RCTEventEmitter {
  ...
  static var shared: RNEventEmitter {
    ...
    logger.debug(
      "RNEventEmitter.shared is accessed but not initialized yet. Waiting for initialization.")
  ...
  @objc func sendEventToNative(_ name: String, payload: [AnyHashable: Any]? = nil) {
    ...
    logger.debug(
      "Event sent to native stream: \(name) with payload: \(String(describing: payload))")
  }
}
